...
contract Casino{
	...
	// Stage 0: Create
	// Initialize
	constructor (uint _betTimePeriodInMinutes, address _humanOracle) public {
		casinoOwnerAdderss = msg.sender;
		humanOracle = _humanOracle;
	...
	function revealWinner(uint _winner) public onlyBy(humanOracle){
		require(now >= bettingTime);
